...
from common.variables import DEFAULT_PORT, DEFAULT_IP_ADDRESS, \
    ...
    RESPONSE_400, REMOVE_CONTACT, RESPONSE_200, USERS_REQUEST, ADD_CONTACT, RESPONSE_511, DATA, PUBLIC_KEY, \
    RESPONSE_205, PUBLIC_KEY_REQUEST
...
class MessengerServerCore(threading.Thread, metaclass=ServerMaker):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                if self.clients:
                    recv_data_lst, self.listen_sockets, self.error_sockets = select.select(self.clients,
                                                                                           self.clients,
                                                                                           [],
                                                                                           0)
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        ...
                        SERVER_LOGGER.debug(f'Клиент {client_with_message.getpeername()} '
